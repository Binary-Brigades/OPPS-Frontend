      ...
      {token !== null ? (
        <div>
          ...
          <div className="p-12 h-full justify-start items-start w-full flex text-black md:pl-[350px]  flex-col  md:px-[200px]  ">
            ...
            <p>Aswer the following questions following the rulles set( Use specified number of words)</p>
            <ul>
              {Questions?.data?.questions?.map((question) => (
                <li className="my-6 bg-[#f5f5f5] p-3 rounded-xl md:min-w-[450px] hover:border-l-blue-500 hover:border-l-4" key={question.id}>
                  <p className="flex flex-col gap-2">
                    <span className="font-semibold w-full">{question.question}</span>
                    <span className="font-light">{question.marks} marks</span>{" "}
                  </p>
                  <textarea
                  ...
                  />
                  <p className="w-full text-right text-sm">maximum words: {question.max_words}</p>
                </li>
              ))}
            </ul>
          </div>
